...
public class ProductTest {
	...
	public void verifyNullIdReturn() {
		Long actualId = product.getId();
		assertNull(actualId, "The returned ID is not NULL when no ID was set");
	}
	...
	public void verifyExtremeValueIdReturn() {
		Long[] extrema = { Long.MAX_VALUE, Long.MIN_VALUE };
		for (Long expectedId : extrema) {
			product.setId(expectedId);
			Long actualId = product.getId();
			assertEquals(expectedId, actualId, "The returned ID doesn't match the set extreme ID");
		}
	}
